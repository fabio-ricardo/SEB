	...
	def EscreveTudo(self,Ta, UR, Z, julianDay):
		...
		for i in xrange(0,self.linhas,self.yBlockSize):
			...
			for j in xrange(0,self.colunas,self.xBlockSize):
				...
				opencl = Opencl()
				inicio_aux = time.time()
				self.img.saidaImagem('ndvi', self.bandaNDVI, self.saidaNDVI, opecl.cl_into_np(formulas.getNDVI()), i, j)
				self.img.saidaImagem('savi', self.bandaSAVI, self.saidaSAVI, opecl.cl_into_np(formulas.getSAVI()), i, j)
				self.img.saidaImagem('iaf', self.bandaIAF, self.saidaIAF, opecl.cl_into_np(formulas.getIAF()), i, j)
				self.img.saidaImagem('albedoSuperficie', self.bandaAlbedoSuper, self.saidaAlbedo, opecl.cl_into_np(formulas.getAlbedoSuper()), i, j)
				
				self.img.saidaImagem('temperaturaSuperficie',self.bandaTempSuper,self.saidaTemp, opecl.cl_into_np(formulas.getTemperaturaSuperficie()), i, j)
				self.img.saidaImagem('saldoRadiacao', self.bandaSaldoRad, self.saidaRad, opecl.cl_into_np(formulas.getSaldoRadiacao()), i, j)
				self.img.saidaImagem('fluxoCalSolo', self.bandaFluxo, self.saidaFluxo, opecl.cl_into_np(formulas.getFluxoCalSolo()), i, j)
				
				self.img.saidaImagem('fracaoEvaporativa', self.bandaFracao, self.saidaFracao, opecl.cl_into_np(formulas.getFracaoEvaporativa()), i, j)

				self.img.saidaImagem('fluxoCalorSensivel', self.bandaCalSen, self.saidaCalSen, opecl.cl_into_np(formulas.getFluxoCalorSensivel()), i,j )
				self.img.saidaImagem('fluxoCalorLatente', self.bandaCalLat, self.saidaCalLat, opecl.cl_into_np(formulas.getFluxoCalorLatente()), i,j)
				self.img.saidaImagem('evapotranspiração24h',self.bandaEvap,self.saidaEvap, opecl.cl_into_np(formulas.getEvapotranspiracao24h()), i,j)
				formulas.passoFinal()
...
class Formulas():
	...
	def __init__(self, Ta, UR, Z, julianDay, img):
		 ...
		 self.opencl = OpenCl()
		 self.nan_array = cl.array.empty(self.opencl.queue,(self.img.rows,self.img.cols), numpy.float32,order='C', allocator=None, data=None)
		 self.nan_array.fill(numpy.nan)
	...
	def reflectanciaParte2(self,NBandas,entrada, j, i, lerColunas, lerLinhas):
		...
		for k in xrange(1,NBandas+1):
			#Applying pyopencl
			dados[k] = self.opencl.np_into_cl(entrada.GetRasterBand(k).ReadAsArray(j,i,lerColunas,lerLinhas).astype(numpy.float32))
			...
			if(k != NBandas and k >= 2):
				valueOff = cl.array.if_positive(self.mask == 0, self.nan_array, dados[k])
		...
		self.t = cl.array.empty(self.opencl.queue,(self.img.rows,self.img.cols), int ,order='C', allocator=None, data=None)
		self.t.fill(True)
		
		self.f =cl.array.empty(self.opencl.queue,(self.img.rows,self.img.cols), int,order='C', allocator=None, data=None)
		self.f.fill(False)
		
		self.mask = cl.array.if_positive(self.mask == 0, self.t, self.f)
		valueOff = None
		dados = None
		#self.entrada = None
	...
	def fazCalculos(self, fracao, fracao2,i, j, lerLinhas, lerColunas):	
		self.noValue = numpy.ones((self.img.rows,self.img.cols))*self.valores.noValue
		self.noValue = self.opencl.np_into_cl(self.noValue)
	...
	def setNdvihdf(self):
		self.ndvi = cl.array.if_positive(self.mask == 0, self.noValue, (self.reflectanciaB2 - self.reflectanciaB1) / (self.reflectanciaB2 + self.reflectanciaB1))
	
	#Sets NDVI for tif files
	def setNdvitif(self):
		self.ndvi = cl.array.if_positive(self.mask == 0, self.noValue, (self.reflectanciaB4 - self.reflectanciaB3) / (self.reflectanciaB4 + self.reflectanciaB3))
	...
	#Sets SAVI
	def setSavi(self):
		self.savi = cl.array.if_positive(self.mask == 0, self.noValue, ((1 + self.valores.L) * (self.reflectanciaB4 - self.reflectanciaB3)) / (self.valores.L + (self.reflectanciaB4 + self.reflectanciaB3)))
	...
	def setIaf(self):
		...
		aux_mask1 = (((0.69 - self.savi) / 0.59) > 0) + self.mask
		self.mask1 = cl.array.if_positive(aux_mask1 < 2 , self.f, self.t)
		
		
		#-----#------#
		self.iaf = cl.array.if_positive(self.mask == 0, self.noValue, -1 * (cl.clmath.log((0.69 - self.savi) / 0.59) / 0.91))
		#-----#------#
		self.mask1 = self.savi <= 0.1
		self.aux = cl.array.empty_like(self.iaf)
		self.aux.fill(0.0)
		self.iaf = cl.array.if_positive(self.mask == 0,self.iaf, self.aux)
	...
	def albedoSuper(self):
		self.albedoSuperficie = cl.array.if_positive(self.mask == 0, self.noValue, (self.albedoPlanetario - self.valores.ap) * self.valores.p2)
		if cl.array.max(self.albedoSuperficie) > self.albedoSupMax:
			self.albedoSupMax = cl.array.max(self.albedoSuperficie)
	...
	def enb_e_e0(self):
		...
		self.aux.fill(0.98)
		self.ENB = cl.array.if_positive(self.mask == 0, self.ENB, self.aux)
		self.E0 = cl.array.if_positive(self.mask == 0, self.E0, self.aux)
	...
	def tempSupertif(self, i, j,lerLinhas, lerColunas):
		#-----#------#
		self.temperaturaSuperficie = cl.array.if_positive(self.mask == 0, self.noValue, self.valores.K2 / cl.clmath.log(((self.ENB * self.valores.K1) / self.radianciaB6) + 1.0))
		#-----#------#
	...
	def tempSuperhdf(self, i,j ,lerLinhas, lerColunas):
		numpy.seterr(all='ignore')
		#Radiance
		T31 =  -1.3250003015 + ((26.2000005725+1.3250003015)/32767)*self.opencl.np_into_cl(self.img.entrada.GetRasterBand(8).ReadAsArray(j,i,lerColunas,lerLinhas).astype(numpy.float32))
		T32 =  -1.2099998176 + ((22.7000145484+1.2099998176)/32767)*self.opencl.np_into_cl(self.img.entrada.GetRasterBand(9).ReadAsArray(j,i,lerColunas,lerLinhas).astype(numpy.float32))
		
		#Temperature
		#-----#------#
		T31 = 1304.4/(cl.clmath.log((self.ENB*729.57/T31)+1))
		T32 = 1197.0/(cl.clmath.log((self.ENB*474.71/T32)+1))
		#-----#------#
	...
	def radOndaLongaEmi(self):
		#-----#------#
		return (self.temperaturaSuperficie**4)*(self.E0 * self.valores.constSB)
		#-----#------#
		
	#Sets Total Radiance
	def setSaldoRadiacao(self):
		self.saldoRadiacao = cl.array.if_positive(self.mask == 0, self.noValue, (self.E0 * self.valores.radOndaLongaInci - self.radOndaLongaEmi()) + ((1.0 - self.albedoSuperficie) * self.valores.radOndaCurtaInci))
	...
	def setFluxoCalSolo(self):
		self.mask1 = self.ndvi < 0
		#-----#------#
		self.fluxoCalSolo = cl.array.if_positive(self.mask == 0, ((self.temperaturaSuperficie - 273.15) * (0.0038 + (0.0074 * self.albedoSuperficie))\
							   * (1.0 - (0.98 * (self.ndvi**4)))) * self.saldoRadiacao, self.valores.G * self.saldoRadiacao)
		#-----#------#
		self.mask1 = None
		self.fluxoCalSolo = cl.array.if_positive(self.mask == 0, self.noValue, self.fluxoCalSolo)
	...
	def getFluxoCalorSensivel(self):
		return cl.array.if_positive(self.mask == 0, self.noValue, (1 - self.fracaoEvaporativa) * (self.saldoRadiacao - self.fluxoCalSolo))
	
	#Gets Latent Heat Balance
	def getFluxoCalorLatente(self):
		return cl.array.if_positive(self.mask == 0, self.noValue, self.fracaoEvaporativa * (self.saldoRadiacao - self.fluxoCalSolo))
		
	#Gets Evapotranspiration in a day
	def getEvapotranspiracao24h(self):
		#-----#------#
		return cl.array.if_positive(self.mask == 0, self.noValue, (self.fracaoEvaporativa * (self.valores.Rg24h * (1.0 - self.albedoSuperficie)\
	...
	def fracaoEvaporativaSSEBI(self, i, j, lerLinhas, lerColunas):
		...
		test1 = albedoSuperficie != self.valores.noValue
		a = numpy.float32(str(self.albedoSupMax))*0.2
		test2 = albedoSuperficie <= a	
		maskAlbedoSuper = test1 + test2
		maskAlbedoSuper = maskAlbedoSuper ==2
		
		limiteLadoEsq = cl.array.if_positive(maskAlbedoSuper == 0, self.noValue, temperaturaSuperficie)
		#limiteLadoEsq = temperaturaSuperficie[maskAlbedoSuper]
		
		test1 = albedoSuperficie != self.valores.noValue
		a = numpy.float32(str(self.albedoSupMax))*0.8
		test2 = albedoSuperficie >= a
		maskAlbedoSuper = test1 + test2
		maskAlbedoSuper = maskAlbedoSuper==2
		
		limiteLadoDir = cl.array.if_positive(maskAlbedoSuper == 0, self.noValue, temperaturaSuperficie)
	...
	def fracao(self):	
		...
		self.fracaoEvaporativa = cl.array.if_positive(self.mask == 0, self.noValue, (c1 + (m1 * self.getAlbedoSuper()) - self.getTemperaturaSuperficie())/((c1 - c2) + ((m1 - m2) * self.getAlbedoSuper())))
	...
	def fracaoEvaporativaSSEB(self, *args):
		...
		self.fracaoEvaporativa = cl.array.if_positive(self.mask == 0, self.noValue, (TH - self.getTemperaturaSuperficie()) / (TH - TC))
